...
raster_folder = "dwd-temp\\data\\extracted\\"  # Change to your folder containing .asc files
geojson_path = "germany-states\\germany-states.geojson"  # Change to your GeoJSON file
output_folder = "dwd-temp\\data\\output_csv"  # Change to your output folder
...
asc_files = glob.glob(os.path.join(raster_folder, "grids_germany_annual_air_temp_mean_*.asc"))
...
for raster_path in asc_files:
    ...
    with rasterio.open(raster_path) as src:
        ...
        for index, row in germany_states.iterrows():
            ...
            min_temp = np.nanmin(temp_values) if temp_values.size > 0 else None
            max_temp = np.nanmax(temp_values) if temp_values.size > 0 else None

            # Append to results
            temperature_data.append({
                "State": state_name,
                "Mean_Temperature (°C)": mean_temp,
    ...
    output_csv_path = os.path.join(output_folder, f"temperature_per_state_{year}.csv")
